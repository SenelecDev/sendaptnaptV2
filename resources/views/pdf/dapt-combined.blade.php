            ...
            <div class="missionContent" id="missionContent">
                ...
                <div class="direction">
                    <table class="table-spacing" style="width: 100%; border-collapse: collapse;">
                        ...
                        <tr>
                            <td style="border: 1px solid #ccc;">
                                <strong>Chargé des travaux :</strong> {{ $demande->charge_travaux_info->nom ?? '-' }}@if($demande->charge_travaux_info && $demande->charge_travaux_info->type === 'externe') <small>(Ext.)</small>@endif<br><br>
                ...
                <p class="footer-text">DAPT {{ $loop->iteration }}/{{ $loop->count }} - Généré le {{ now()->format('d/m/Y à H:i') }}</p>
            </div>
        </div>
    </div>
    @endforeach
