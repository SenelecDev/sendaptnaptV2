...
<div class="space-y-6">
    ...
    <div class="card-senelec p-4">
        <form action="{{ route('admin.users.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div>
                <select name="groupe_id" class="input-senelec">
            ...
            <table class="table-senelec min-w-[900px]">
                <thead>
                    <tr>
                        ...
                        <th class="min-w-[150px]">Groupe</th>
                ...
                <tbody>
                    @forelse($users as $user)
                        <tr>
                            ...
                            <td>
                                <p class="text-sm text-gray-900">{{ $user->groupe?->nom ?? '-' }}</p>
                            </td>
                            <td>
                                ...
                                <div class="flex items-center justify-end space-x-1">
                                    @if(auth()->user()->hasRole('admin') && !$user->hasRole('admin') && auth()->id() !== $user->id)
                                        <form action="{{ route('admin.impersonate.start', $user) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit" 
                                                   class="p-1.5 text-indigo-500 hover:text-indigo-700 hover:bg-indigo-50 rounded-lg transition-colors" title="Simuler">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @if($user->id !== auth()->id() && !$user->ldap_guid && !$user->oracle_person_id)
