    ...
    <div class="card-senelec p-4">
        <form method="GET" action="{{ route('admin.activity-log.index') }}" class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.activity-log.index') }}" class="btn-senelec-outline text-sm px-4 py-2">Reset</a>
            </div>
        </form>
    </div>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 whitespace-nowrap">
                            @switch($activity->log_name)
                                @case('users')
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800">Users</span>
                                    @break
                                @case('demandes')
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 text-purple-800">DAPT</span>
                                    @break
                                @case('notes')
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800">NAPT</span>
                                    @break
                                @default
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800">{{ $activity->log_name }}</span>
                            @endswitch
                        </td>
                        ...
                        <td class="px-4 py-3 text-sm text-gray-900">
                            @if($activity->subject)
                                @php
                                    ...
                                    } elseif ($activity->subject instanceof \App\Models\Demande) {
                                        $subjectLabel = $activity->subject->numero_demande ?? 'DAPT #' . $activity->subject->id;
                                    } elseif ($activity->subject instanceof \App\Models\Note) {
                                        $subjectLabel = $activity->subject->numero_note ?? 'NAPT #' . $activity->subject->id;
                                    } else {
