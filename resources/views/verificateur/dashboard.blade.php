...
@section('title', 'Tableau de bord - Vérificateur')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-gray-600" style="margin-top: 15px;">Notes d'Arrêt Pour Travaux à vérifier et suivi</p>
        </div>
    ...
    <!-- Statistiques - 7 compteurs -->
    <div class="flex flex-wrap md:flex-nowrap gap-4 overflow-x-auto pb-2">
        <a href="{{ route('verificateur.notes.index', ['statut' => 'en attente de vérification']) }}" class="card-senelec p-4 border-l-4 border-yellow-500 hover:scale-105 transition-transform cursor-pointer flex-1 min-w-[140px]">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    <p class="text-xs font-medium text-gray-500">À vérifier</p>
                    <p class="text-xl font-semibold text-gray-900">{{ $stats['en_attente_verification'] }}</p>
                </div>
            </div>
        </a>
        <a href="{{ route('verificateur.notes.index', ['statut' => 'vérifiée']) }}" class="card-senelec p-4 border-l-4 border-teal-500 hover:scale-105 transition-transform cursor-pointer flex-1 min-w-[140px]">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    <p class="text-xs font-medium text-gray-500">Vérifiées</p>
                    <p class="text-xl font-semibold text-gray-900">{{ $stats['verifiees'] }}</p>
                </div>
            </div>
        </a>
        <a href="{{ route('verificateur.notes.index', ['statut' => 'validée']) }}" class="card-senelec p-4 border-l-4 border-green-500 hover:scale-105 transition-transform cursor-pointer flex-1 min-w-[140px]">
            <div class="flex items-center">
        ...
        <a href="{{ route('verificateur.notes.index', ['statut' => 'en cours d\'exécution']) }}" class="card-senelec p-4 border-l-4 border-orange-500 hover:scale-105 transition-transform cursor-pointer flex-1 min-w-[140px]">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    <p class="text-xs font-medium text-gray-500">Exécution</p>
                    <p class="text-xl font-semibold text-gray-900">{{ $stats['en_cours_execution'] }}</p>
                </div>
            </div>
        </a>
        <a href="{{ route('verificateur.notes.index', ['statut' => 'retournée']) }}" class="card-senelec p-4 border-l-4 border-red-500 hover:scale-105 transition-transform cursor-pointer flex-1 min-w-[140px]">
            <div class="flex items-center">
        ...
        <a href="{{ route('verificateur.notes.index', ['statut' => 'annulée']) }}" class="card-senelec p-4 border-l-4 border-gray-500 hover:scale-105 transition-transform cursor-pointer flex-1 min-w-[140px]">
            <div class="flex items-center">
        ...
        <a href="{{ route('verificateur.notes.index', ['statut' => 'exécutée']) }}" class="card-senelec p-4 border-l-4 border-emerald-500 hover:scale-105 transition-transform cursor-pointer flex-1 min-w-[140px]">
            <div class="flex items-center">
        ...
        <div class="card-senelec p-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                ...
                <div class="flex gap-2">
                    <a href="{{ route('verificateur.dashboard', ['periode' => 'semaine']) }}" 
                    ...
                    <a href="{{ route('verificateur.dashboard', ['periode' => 'mois']) }}" 
                    ...
                    <a href="{{ route('verificateur.dashboard', ['periode' => 'annee']) }}" 
    ...
    <div class="card-senelec overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
            <h3 class="text-lg font-semibold text-gray-900">Dernières notes à vérifier</h3>
            <a href="{{ route('verificateur.notes.index', ['statut' => 'en attente de vérification']) }}" class="text-senelec-purple hover:text-purple-700 text-sm font-medium">
                Voir tout →
            </a>
        </div>
                    ...
                    @forelse($dernieresNotes as $note)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="{{ route('verificateur.notes.show', $note) }}" 
                                   class="inline-flex items-center px-3 py-1.5 bg-senelec-purple text-white text-xs font-medium rounded-md hover:bg-purple-700 transition-colors" title="Vérifier">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                                ...
                                <p class="mt-2 text-lg font-medium">Aucune note à vérifier</p>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Couleurs
        const colors = {
            en_attente_verification: { bg: 'rgba(234, 179, 8, 0.2)', border: 'rgb(234, 179, 8)' },
            verifiees: { bg: 'rgba(20, 184, 166, 0.2)', border: 'rgb(20, 184, 166)' },
        ...
        if (evolutionCtx) {
            new Chart(evolutionCtx.getContext('2d'), {
                ...
                data: {
                    ...
                    datasets: Object.keys(graphData.datasets).map(key => ({
