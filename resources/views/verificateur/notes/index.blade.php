...
@section('title', 'Notes - Vérificateur')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-gray-600">Liste des NAPT à vérifier et suivre</p>
        </div>
    </div>

    <!-- Raccourcis par statut -->
    <div class="flex flex-wrap gap-2">
        <a href="{{ route('verificateur.notes.index', ['statut' => 'tous']) }}" 
        ...
        <a href="{{ route('verificateur.notes.index', ['statut' => 'en attente de vérification']) }}" 
           class="px-3 py-1.5 rounded-full text-sm font-medium transition-colors {{ $statut == 'en attente de vérification' ? 'bg-yellow-500 text-white' : 'bg-yellow-100 text-yellow-800 hover:bg-yellow-200' }}">
            À vérifier
        </a>
        <a href="{{ route('verificateur.notes.index', ['statut' => 'vérifiée']) }}" 
           class="px-3 py-1.5 rounded-full text-sm font-medium transition-colors {{ $statut == 'vérifiée' ? 'bg-teal-500 text-white' : 'bg-teal-100 text-teal-800 hover:bg-teal-200' }}">
            Vérifiée
        </a>
        <a href="{{ route('verificateur.notes.index', ['statut' => 'validée']) }}" 
        ...
        <a href="{{ route('verificateur.notes.index', ['statut' => 'en cours d\'exécution']) }}" 
        ...
        <a href="{{ route('verificateur.notes.index', ['statut' => 'exécutée']) }}" 
        ...
        <a href="{{ route('verificateur.notes.index', ['statut' => 'retournée']) }}" 
        ...
        <a href="{{ route('verificateur.notes.index', ['statut' => 'annulée']) }}" 
    ...
    <div class="card-senelec p-6">
        <form method="GET" action="{{ route('verificateur.notes.index') }}" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <select name="statut" class="input-senelec w-full">
                        ...
                        <option value="en attente de vérification" {{ $statut == 'en attente de vérification' ? 'selected' : '' }}>En attente de vérification</option>
                        <option value="vérifiée" {{ $statut == 'vérifiée' ? 'selected' : '' }}>Vérifiée</option>
                ...
                <div class="flex items-end gap-2">
                    ...
                    <a href="{{ route('verificateur.notes.index') }}" class="btn-senelec-outline px-4" title="Réinitialiser">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($notes as $note)
                        <tr class="hover:bg-gray-50 cursor-pointer" onclick="window.location='{{ route('verificateur.notes.show', $note) }}'">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @php
                                    ...
                                    $statusLabels = [
                                        'en attente de vérification' => 'À vérifier',
                                        'vérifiée' => 'Vérifiée',
                                        'validée' => 'Validée',
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium" onclick="event.stopPropagation()">
                                <div class="flex items-center justify-end gap-2">
                                    @if(in_array($note->statut, ['en attente de vérification', 'retournée']))
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                                ...
                                <p class="mt-1 text-sm">Aucune note à vérifier pour le moment.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
